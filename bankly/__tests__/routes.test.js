...
describe("POST /auth/login", function () {
  ...
  // TESTS BUG #1
  test("should raise 401 for incorrect username/password", async function () {
    const response = await request(app).post("/auth/login").send({
      username: "DNE",
      password: "DNE",
    });
...
describe("GET /users", function () {
  ...
  // TESTS BUG #4
  test("should deny acces if invalid token is provided", async function () {
    const response = await request(app)
      .get("/users")
      .send({
        _token: jwt.sign({ user: "fake user", admin: false }, "fake key"),
      });
    expect(response.statusCode).toBe(401);
  });
...
describe("GET /users/[username]", function () {
  ...
  // TESTS BUG #2
  test("should raise 404 if username not found", async function () {
    const response = await request(app)
      .get("/users/DNE")
      .send({ _token: tokens.u1 });
